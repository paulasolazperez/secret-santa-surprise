      ...
      <div className="relative z-10 container mx-auto px-4 py-16 flex flex-col items-center">
        <header className="text-center mb-16 animate-float">
          <div className="relative inline-block mb-6">
            ...
            <Sparkles className="w-6 h-6 text-gold-light absolute -bottom-1 -left-3 animate-twinkle" style={{ animationDelay: '0.5s' }} />
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-4xl w-full">
          <FeatureCard
            ...
            title="Crea tu grupo"
            description="Invita a tus amigos y familia con un código único. Cada uno accede desde cualquier lugar."
          />
          <FeatureCard
            icon={<Shuffle className="w-10 h-10" />}
            title="Sorteo automático"
            description="Con un clic, el sistema asigna aleatoriamente a quién regala cada participante."
          />
          <FeatureCard
            icon={<Eye className="w-10 h-10" />}
            title="Secreto garantizado"
            description="Solo tú puedes ver a quién te toca regalar. ¡Nadie más lo sabrá!"
          />
        </div>

        <footer className="mt-20 text-center text-muted-foreground text-sm">
          <p>Hecho con ❄️ para hacer tu Navidad más divertida</p>
        </footer>
      </div>
    </div>
  );
